...
// File @openzeppelin/contracts/utils/Base64.sol@v4.8.0
...
// File contracts/japanDream/OnimaruJapanDream.sol

pragma solidity ^0.8.4;

contract OnimaruJapanDream is Ownable, ERC721A, ReentrancyGuard, ERC2981, DefaultOperatorFilterer {
    address payable internal _withdrawWallet;

    // service fee 2.5%
    uint256 public constant allowListPrice = 0.08 ether * 1.025;
    uint256 public constant flashSalePrice = 0.08 ether * 1.025;
    uint256 public constant publicSalePrice = 0.1 ether * 1.025;
    ...
    ) ERC721A('Onimaru: Japan Dream', 'ONIMARUJD') {
    ...
    function allowListMint(uint256 quantity, bytes32[] calldata proof) external payable nonReentrant {
        require(isAllowListSaleEnabled, 'Allow List Mint Is Paused');
        require(
            MerkleProof.verify(proof, _allowListMerkleRoot, keccak256(abi.encodePacked(msg.sender))),
            'MerkleTree Proof Failed'
        );
        require(quantity <= maxMintsForAllowList, 'Exceed Max Allowed Mint Amount');
        require(_allowListMinted[msg.sender] + quantity <= maxMintsForAllowList, 'Have No Allow List Mint Left');
        require(msg.value == allowListPrice * quantity, 'Incorrect Eth Value');
        require(quantity + totalSupply() <= MAX_SUPPLY, 'No More NFTs');
    ...
    function flashSaleMint(uint256 quantity, bytes32[] calldata proof) external payable nonReentrant {
        require(isFlashSaleEnabled, 'Flash Sale Mint Is Paused');
        require(
            MerkleProof.verify(proof, _allowListMerkleRoot, keccak256(abi.encodePacked(msg.sender))),
            'MerkleTree Proof Failed'
        );
        require(quantity <= maxMintsForFlashSale, 'Exceed Max Flash Sale Mint Amount');
        require(_flashSaleMinted[msg.sender] + quantity <= maxMintsForFlashSale, 'Have No Flash Sale Mint Left');
        require(msg.value == flashSalePrice * quantity, 'Incorrect Eth Value');
        require(quantity + totalSupply() <= MAX_SUPPLY, 'No More NFTs');
    ...
    function publicMint(uint256 quantity) external payable nonReentrant {
        require(isPublicSaleEnabled, 'Public Sale Mint Is Paused');
        require(quantity <= maxMintsForPublicSale, 'Exceed Max Allowed Mint Amount');
        require(_publicSaleMinted[msg.sender] + quantity <= maxMintsForPublicSale, 'Have No Public Mint Left');
        require(msg.value == publicSalePrice * quantity, 'Incorrect Eth Value');
        require(quantity + totalSupply() <= MAX_SUPPLY, 'No More NFTs');
